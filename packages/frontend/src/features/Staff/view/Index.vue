  ...
  <section class="flex" :class="{ 'gap-5' : isSidePage }">
    <div class="w-full transition ease-in-out" :class="{ '!w-3/4' : isSidePage }">
      ...
      <div class="mt-10 py-5 border-b border-primary-50 overflow-hidden">
        <el-table style="width: 100%" v-loading="loading" height="550" stripe :data="staff">
          ...
          <el-table-column prop="email" label="Email" width="400" show-overflow-tooltip>
            <template #default="{ row }">
              {{ row.email }}
            </template>
          </el-table-column>
          ...
          <el-table-column prop="isActive" label="Active" width="100">
            <template #default="{ row }">
              <el-tag>{{ row.isActive ? 'Active' : 'No active' }}</el-tag>
            </template>
          </el-table-column>
        ...
        <div class="flex justify-end mt-5">
          <el-pagination v-model:current-page="meta.page" v-model:page-size="meta.size" :page-sizes="[10, 20, 30, 40]"
            background layout="sizes, prev, pager, next" :total="meta.total" @size-change="(size: any) => {
              meta.size = size;
              getStaff();
